# Deploy the Direct-Only Backend on a Dedicated RTX 3060 PC

This branch runs the direct-chat product on an Ubuntu 24.04 LTS PC. It contains
no upload, OCR, layout, embedding, vector retrieval, or RAG runtime.
...
A dedicated server installs each component once:
...
sudo git clone --branch sever --single-branch \
  https://github.com/phamtu2x5/ielts-chatbot.git /opt/ielts-chatbot
...
sudo -u ielts-chatbot /opt/ielts-chatbot/backend/.venv/bin/pip install --no-cache-dir \
  -r /opt/ielts-chatbot/backend/requirements.txt
...
Run `pip install --no-cache-dir -r requirements.txt` during an update
